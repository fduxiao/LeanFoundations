...
/-!
`inductive` is one way to extend the **judgement rules**. `:` can be considered as the
relation on `terms` and `types` freely generated by those rules. In this case, the
relation `: Day` is freely generated by **constructors** `Monday`, `Tuesday`, `Wednesday`, etc.
Besides, we also added a judgement `Day: Type` indicating that `Day` is a type.
-/
...
/-!
This is equivalent to define a type `scratch.Bool` with constructors
`scratch.Bool.true` and `scratch.Bool.false`.
-/

#check scratch.Bool
#check scratch.Bool.true
...
/-!
We added a new judgement `next_day: Day -> Day` here.
In fact, it is the following judgement
```
fun (w: Day) => match w with ...: Day -> Day
```
given a name `next_day` by _command_ `def`.
The rule of this judgement can be concluded as follows:
> - For two types `A` and `B`, we can form a new type `A -> B`.
> - To find a term of type `A -> B`, we use `λ (x: A) => M` where `M: B` is a term of
>   type `B` that may depend on `x: A`.
> - In our case, the `M` is constructed by the `match`, which is a rule to construct a term
    out of an enumeration.

On the other hand, we can also `def` a name for _function application_.
-/
def next_day_of_sunday: Day := next_day Day.Sunday
#eval next_day_of_sunday

/-!
> The rule here is that if you have `f: A -> B` and `a: A`, then you can form a new term `f a: B`.
-/

/-!
It is your turn to write a function now. The `sorry` is a placeholder for a term of the specified type.
The rule is _I don't know how to write a term of this type, so just admit it_. Since this may also
introduce some inconsistency, Lean will complain about it.
-/
def is_monday: Day -> Bool := fun (w: Day) =>
  match w with
  | .Monday => true
  | _ => false  -- `_` is for the wildcard case, i.e., all other than `.Monday`


def is_tuesday: Day -> Bool := fun w =>  -- the type `: Day` can be omitted since it can be inferred from the context
  match w with
  | .Tuesday => true
  | _ => false


-- Try to define a function to check whether a day is Wednesday or not.
def is_wednesday: Day -> Bool := fun w =>
  solution[[
    match w with
    | .Wednesday => true
    | _ => false
...
def day_eq_b: Day -> Day -> Bool := fun d1 =>
  match d1 with
  | .Monday => is_monday  -- we can use the function `is_monday` defined above
  | .Tuesday => is_tuesday  -- again, we use the function `is_tuesday` defined above
  | .Wednesday => fun d2 =>  -- you can also define a function by λ-abstraction directly
     match d2 with
    | .Wednesday => true
    | _ => false
  | .Thursday => fun d2 =>
...
#eval Day.Wednesday.next.next.next.next  -- Or, you have to type `Day.next (Day.next (Day.next (Day.next Day.Wednesday)))`
...
definition, this also means a new judgement is added while the rules are
tactics -- we will gradually reveal the magic of them. For now, let's look at the following example,
which means that we have a judgement `not_false_is_true: Bool.not .false = .true`.
